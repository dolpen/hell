...
public class Res extends Model {
    ...
    public static List<Res> getPublicResList(Village village, int dayCount, int limit) {
        if (limit > 0) return find(
                "villageId = ?1 and (permission = ?2 or (permission = ?3 and skill = ?4 and logType = ?5)) and dayCount = ?6 order by id desc",
                village.villageId, Permission.Public, Permission.Group, Skill.Werewolf, LogType.Say, dayCount).fetch(limit);
        return find(
                "villageId = ?1 and (permission = ?2 or (permission = ?3 and skill = ?4 and logType = ?5)) and dayCount = ?6 order by id asc",
    ...
    public static List<Res> getPersonalResList(Village village, Member member, int dayCount, int limit) {
        if (limit > 0) return find(
                "villageId = ?1 and (memberId = ?2 or (permission = ?3 and skill in (?4)) or (permission = ?3 and skill = ?5 and logType = ?6) or permission = ?7) and dayCount = ?8 order by id desc",
        ...
        return find(
                "villageId = ?1 and (memberId = ?2 or (permission = ?3 and skill in (?4)) or (permission = ?3 and skill = ?5 and logType = ?6) or permission = ?7) and dayCount = ?8 order by id asc",
                village.villageId, member.memberId, Permission.Group,
    ...
    public static List<Res> getDeadPersonalResList(Village village, Member member, int dayCount, int limit) {
        if (limit > 0) return find(
                "villageId = ?1 and (memberId = ?2 or (permission = ?3 and skill in (?4)) or (permission = ?3 and skill = ?5 and logType = ?6) or permission in (?7)) and dayCount = ?8 order by id desc",
                village.villageId, member.memberId, Permission.Group,
        ...
        return find(
                "villageId = ?1 and (memberId = ?2 or (permission = ?3 and skill in (?4)) or (permission = ?3 and skill = ?5 and logType = ?6) or permission in (?7)) and dayCount = ?8 order by id asc",
                village.villageId, member.memberId, Permission.Group,
    ...
    public static List<Res> getAllResList(Village village, int dayCount, int limit) {
        if (limit > 0)
            return find("villageId = ?1 and dayCount = ?2 order by id desc", village.villageId, dayCount).fetch(limit);
        return find("villageId = ?1 and dayCount = ?2 order by id asc", village.villageId, dayCount).fetch();
    }
